...
class SE_CSS extends SC_Element {
	...
	static function getPath($file, $flavour = null, $directory = null) {
		...
		if(!isset(self::$files_exists[$token_id])) {
			...
			if(!$flavour) $flavour = SC_Main::${ self::type .'_FLAVOUR'};
			if(!$directory) $directory = self::default_dir;
			
			$bases = array(
				'SimplOn' => SC_Main::$SimplOn_PATH,
				'Site' => SC_Main::$LOCAL_ROOT,
			);
	...
	function index() {
		...
		readfile(SC_Main::$SimplOn_PATH.'/'. self::type .'/'.$this->flavour.'/'.$this->directory.'/'.$this->file);
	}
}