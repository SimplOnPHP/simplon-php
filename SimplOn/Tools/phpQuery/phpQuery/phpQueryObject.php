...
//			var_dump(phpQuery::$documents);
			throw new SC_Exception("Document with ID '{$id}' isn't loaded. Use phpQuery::newDocument(\$html) or phpQuery::newDocumentFile(\$file) first.");
		}
		$this->documentID = $id;
...
//				)->elements;
			...
			case 'only-child':
				$this->elements = $this->map(
					//fixRSL 2023 create_function('$node','return pq($node)->siblings()->size() == 0 ? $node : null;')
					function($node) {return pq($node)->siblings()->size() == 0 ? $node : null;} //fixRSL 2023
				)->elements;
			break;
			case 'first-child':
				$this->elements = $this->map(
					//fixRSL 2023 create_function('$node', 'return pq($node)->prevAll()->size() == 0 ? $node : null;')
					function($node) {
						return pq($node)->prevAll()->size() == 0 ? $node : null;
					} //fixRSL 2023
					//fixRSL 2023 create_function('$matches', "return '$delimiter' . strtolower(\$matches[1]);");
					//fixRSL 2023 function($matches) use ($delimiter) {return $delimiter . strtolower($matches[1]);};

				)->elements;
			break;
			case 'last-child':
				$this->elements = $this->map(
					//fixRSL 2023 create_function('$node', 'return pq($node)->nextAll()->size() == 0 ? $node : null;')
					function($node) {return pq($node)->nextAll()->size() == 0 ? $node : null;} //fixRSL 2023
				)->elements;
			break;
			case 'nth-child':
				$param = trim($args, "\"'");
...
//						);
					...
					case 'before':
						if (! $toNode->parentNode)
							throw new SC_Exception("No parentNode, can't do {$type}()");
						else
...
//			return array();
	...
	public function __call($method, $args) {
		...
		if (isset(phpQuery::$extendMethods[$method])) {
			...
			throw new SC_Exception("Method '{$method}' doesnt exist");
	}
	/**
	 * Safe rename of next().
	 *
