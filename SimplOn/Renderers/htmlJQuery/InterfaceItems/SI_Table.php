...
class SI_Table extends SI_Container {
    ...
    function fillLayout( voku\helper\HtmlDomParser $dom){
        ...
        if(!empty($this->items)){  
            ...
            $headRow = $headerRowCell[0].L('There are no elements to show').$headerRowCell[1]; 
            $headerRow = $headerRow[0].$headRow.$headerRow[1]; 

            $rowHTML = $colCell[0].$colTitleInCell[0].' &nbsp;'.$colTitleInCell[1].$colValInCell[0].L('There are no elements to show').$colValInCell[1].$colCell[1];
            $rowHTML= $row[0].$rowHTML.$row[1];
