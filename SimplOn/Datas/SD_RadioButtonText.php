...
class SD_RadioButtonText extends SD_String
{
    /**
     *
	...
	public function showInput($fill=true)
	{
		...
		foreach($this->options as $value){
			$ret.='<div>';
			$ret.=($this->showValues ? $value:'').'<input class="SimplOn input '. $this->getClass() .'" name="'. $this->name() .'"  value="'.$value.'"'.' '.(($fill && $this->val==$value)?' checked="checked"':'').' type="radio" />  ';
			$ret.='</div>';
		}
		return  $ret; 
	}
