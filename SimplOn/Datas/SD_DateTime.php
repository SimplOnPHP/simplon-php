 ...
 * 
 * 
 * TODO: ensure that changing from date to datetime changes the colum type in the mysqls DBs
 */
class SD_DateTime extends SD_String {
    /**
     *
	...
	function val($val = null) {
		...
		if(isset($val) && $val != '0000-00-00 00:00:00') {
			if(!$this->fixedValue) {
				...
				} else {//if val isn't empty and is required try to verify if $val is numeric then stores it into $dataObj but if the try block fail then throw an exception
					try {
						if(is_numeric($val)) {
							$dateObj = new \DateTime();
							$dateObj->setTimestamp($val);
						} else if(strpos($val,'-')) {
							$dateObj = DateTime::createFromFormat($this->dbFormat, $val);
						} else {
							$val = str_replace([',',' a. m.',' p. m.'],['',' am',' pm'],$val);
							$dateObj = DateTime::createFromFormat($this->viewFormat, $val);
