 ...
 * an specific method or to an specific class SD_from another element.   
 * 
 * 
...
class SD_SimplOnLink extends SD_ComplexData {
	...
	public function __construct($label, array $sources, $class, array $construct_params = array(), $method = 'showView', array $method_params = array(), $flags=null, $searchOp=null){
		$this->class = $class;//class SD_name to access
	...
	public function val($sources = null){
           ...
           // @href save the URL to the class SD_indicated with the parameters to initialize it
			$redender = $GLOBALS['redender'];
			$href = $redender->encodeURL($this->class, $this->construct_params, $this->method, $this->method_params);
			...
			$redender = $GLOBALS['redender'];
            return $redender->link($content, $href);
	}
