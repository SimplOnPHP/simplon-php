 ...
 * @category Data
 */
class SD_Email extends SD_String {
    /**
     *
            ...
            if(isset($val)) {
                if(!$this->fixedValue) {
                    ...
                    if(!$val && $this->required) {
                        throw new SC_DataValidationException ($this->validationRequired);
                    }else if(self::is_email($val)) { // if $val isn't empty and is required check if it's a correct email
                        $this->val = $val;
                    }else { //if it's not a correct email throw an exception
                        throw new SC_DataValidationException ($this->validationEmail);
                    }
                }
            }else { //if $val is null return the same variable
            	return $this->val;
            }
	}
	/**
