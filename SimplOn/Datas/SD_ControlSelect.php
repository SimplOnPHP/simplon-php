 ...
 * @category Data
 */
class SD_ControlSelect extends SD_ComplexData {
	...
	public function getJS($method) {
		...
		$select_js1 = SE_JS::getPath("11-ui.multiselect.js");
		$select_js2 = SE_JS::getPath("12-ui-multiselect-es.js");
		$array_js[] = $select_js1;
		$array_js[] = $select_js2;
		return $array_js;
	}
	...
	public function addCount($sources=  array()) {
		...
		foreach ($sources as $value) {
			$this->parent->addData('count'.$counter , new SD_Count('count('.$value.')',$value)); 
			$counter +=1;
		}
	}
	...
	public function addSum($sources = array()) {
		$counter = 0;
		foreach ($sources as $value) {
			$this->parent->addData('sum'.$counter , new SD_Sum('total('.$value.')',$value));
			$counter +=1;
		}
	}	
}
