 ...
 * correct date to put in the input, This date must be latest than today. 
 * 
 ...
 * @category Data
 */
class SD_TimeTo extends SD_String {
    /**
     *
                    ...
                    try {
                            ...
                            $dateObj->setTimestamp($val);
                                        ...
                                        if($this->timePrecision == 'hours' || $this->timePrecision == 'minutes' || $this->timePrecision == 'seconds'){
                                            ...
                                            $limitDate = new \DateTime("tomorrow");
                                        }
                                        if($dateObj < $limitDate){
                                            throw new SC_DataValidationException ();
                                        }
                    } catch(\Exception $e) {
                        throw new SC_DataValidationException ($this->validationDate);
                    }
 ...
 */	
	function showView($template = null){
            return '<div id="'.$this->ObjectId().'"></div><script type="text/javascript">timeToTimer("'.$this->viewVal.'", "'.$this->orderFormatFlags($this->timerFormat).'", "'.$this->ObjectId().'")</script>';
	}
	
	public function showInput($fill=true) {
            return 
            ($this->label() ? '<label for="'.$this->htmlId().'">'.$this->label().': </label>' : '') .
            '<input id="'.$this->htmlId().'" class="'.$this->htmlClasses('timeTo').' '.$this->timePrecision.'" name="'.$this->inputName().'" '.(($fill)?'value="'.$this->viewVal.'"':'').' type="text" autocomplete="off"/>';
	}
}